...
for cent in centers	:
	...
	print(f"adgpu --lfile {i+1}_{ligand} --ffile {i+1}_{protname}.maps.fld --gbest 1 --npdb {ifgenrun} --nrun {nrun} -p 2048 --rmstol 3 --derivtype Zn=ZN --flexres {flexres} ")
	if HEURMAX is None:
		os.system(f"adgpu --lfile {i+1}_{ligand} --ffile {i+1}_{protname}.maps.fld --gbest 1 --npdb {ifgenrun} --nrun {nrun} -p 2048 --rmstol 3 --derivtype Zn=ZN --flexres {flexres}")
	else:
		os.system(f"adgpu --lfile {i+1}_{ligand} --ffile {i+1}_{protname}.maps.fld --gbest 1 --npdb {ifgenrun} --nrun {nrun} -p 2048 --rmstol 3 --derivtype Zn=ZN --flexres {flexres} --heurmax {HEURMAX} ")
